        ...
        try {
            switch (requestCode){
                case VICE_LICENSE_CODE:
                    Logger.getInstance(getApplicationContext()).info("相机回调成功,data is : " + data);
...
//                    Bitmap photoBmp = getBitmapFormUri(this, Uri.fromFile(file));
                    ...
                    Logger.getInstance(getApplicationContext()).info("newbitmap is :" + newbitmap);
                    addViceLicernseImage.setImageBitmap(newbitmap);
